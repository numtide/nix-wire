...
fn split_directions(records: &[nix_wire_recording::Record]) -> (Vec<u8>, Vec<u8>) {
...
fn load_fixture(name: &str) -> Vec<nix_wire_recording::Record> {
...
async fn daemon_only_handshake_matches_full_handshake() {
    ...
    let daemon_info = protocol::parse_daemon_handshake(&mut daemon2, full_info.client_version)
        .await
        .unwrap();
