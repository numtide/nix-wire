...
use nix_wire_recording::{Direction, Record, RecordingReader};
...
async fn pre_parse_recording(records: &[Record]) -> Result<PreParseResult> {
    ...
    loop {
        ...
        if stderr_result.terminal == Some(StderrCode::Last) {
            if let Some(o) = op {
                let _ = protocol::skip_daemon_result(o, negotiated_version, &mut daemon).await;
            }
        }
...
async fn receive_daemon_responses<R: tokio::io::AsyncRead + Unpin>(
    ...
    let live_info = protocol::parse_daemon_handshake(&mut daemon, pre_parse.client_version).await?;
    let live_negotiated = std::cmp::min(pre_parse.client_version, live_info.server_version);
    ...
    for (i, op) in pre_parse.ops.iter().enumerate() {
        ...
        if stderr_result.terminal == Some(StderrCode::Last) {
            if let Some(o) = op {
                if let Err(e) = protocol::skip_daemon_result(*o, live_negotiated, &mut daemon).await
                {
                    warn!(
