...
/// Write a StringSet (set of strings) to the wire protocol.
///
/// Format: `[u64 count][string1][string2]...`
pub fn write_string_set(w: &mut impl Write, strings: &[String]) -> Result<()> {
    write_u64(w, strings.len() as u64)?;
    for s in strings {
        write_string(w, s)?;
    }
    Ok(())
...
/// Write framed data to the wire protocol.
///
/// Splits `data` into chunks of `chunk_size` bytes (default 64 KiB),
/// each prefixed with `[u64 length]`, then writes a zero-length terminator.
pub fn write_framed(w: &mut impl Write, data: &[u8]) -> Result<()> {
    const CHUNK_SIZE: usize = 64 * 1024;
    for chunk in data.chunks(CHUNK_SIZE) {
        write_u64(w, chunk.len() as u64)?;
        w.write_all(chunk)
            .context("failed to write framed chunk data")?;
    }
...
mod tests {
    ...
    #[test]
    fn roundtrip_write_string_set() {
        let mut buf = Vec::new();
        let strings = vec!["foo".to_string(), "bar".to_string()];
        write_string_set(&mut buf, &strings).unwrap();
        let set = read_string_set(&mut Cursor::new(&buf)).unwrap();
        assert_eq!(set, strings);
    }

    #[test]
    fn roundtrip_write_framed() {
        let data = b"hello world, this is framed data";
        let mut buf = Vec::new();
        write_framed(&mut buf, data).unwrap();

        // Read back: chunks until zero terminator
        let mut cursor = Cursor::new(&buf);
        let mut result = Vec::new();
        loop {
            let chunk_len = read_u64(&mut cursor).unwrap();
            if chunk_len == 0 {
                break;
            }
    ...
    #[test]
    fn write_framed_empty() {
        let mut buf = Vec::new();
        write_framed(&mut buf, b"").unwrap();
        // Should just be the zero terminator
