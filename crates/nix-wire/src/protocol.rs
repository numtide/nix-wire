...
use crate::ops::Op;
use crate::script::{ErrorInfo, FramedData, OpCall, PathInfoResult, ResultData};
    ...
    D: AsyncBufRead + Unpin,
{
    ...
    if StderrCode::from_u64(code_val) != Some(StderrCode::Last) {
        bail!("expected STDERR_LAST after handshake, got {code_val:#x}",);
    }
...
pub async fn read_stderr_loop<D>(daemon: &mut AsyncWireReader<D>) -> Result<StderrResult>
where
    D: AsyncBufRead + Unpin,
{
    let mut count = 0u64;
...
async fn skip_stderr_payload<D>(daemon: &mut AsyncWireReader<D>, code: StderrCode) -> Result<()>
where
    D: AsyncBufRead + Unpin,
{
    match code {
...
async fn skip_optional_duration<D>(daemon: &mut AsyncWireReader<D>) -> Result<()>
where
    D: AsyncBufRead + Unpin,
{
...
/// Read the arguments for a known op from the client stream, returning an OpCall.
///
/// Returns None for ops whose arguments are too complex to parse
/// (BuildDerivation, AddToStoreNar) -- caller should fall back to raw bytes.
pub async fn read_op_args<C>(
    op: Op,
    version: ProtocolVersion,
    client: &mut AsyncWireReader<C>,
) -> Result<Option<OpCall>>
where
    C: AsyncBufRead + Unpin,
{
    match op {
        ...
        | Op::NarFromPath => {
            let path = client.read_string().await?;
            Ok(Some(OpCall::SinglePath { op, path }))
        }

        // Single string argument
        Op::AddIndirectRoot | Op::QueryPathFromHashPart => {
            let s = client.read_string().await?;
            Ok(Some(OpCall::SingleString { op, value: s }))
        }

        // QueryValidPaths: StorePathSet + substitute flag (>= 1.27)
        Op::QueryValidPaths => {
            let paths = client.read_string_set().await?;
            let substitute = if version >= ProtocolVersion::new(1, 27) {
                client.read_u64().await? != 0
            } else {
                false
            };
            Ok(Some(OpCall::PathSetFlag {
                op,
                paths,
                substitute,
            }))
        ...
        Op::QuerySubstitutablePaths => {
            let paths = client.read_string_set().await?;
            Ok(Some(OpCall::PathSet { op, paths }))
        }

        // SetOptions
        Op::SetOptions => {
            let keep_failed = client.read_u64().await?;
            let keep_going = client.read_u64().await?;
            let try_fallback = client.read_u64().await?;
            let verbosity = client.read_u64().await?;
            let max_build_jobs = client.read_u64().await?;
            let max_silent_time = client.read_u64().await?;
            let use_build_hook = client.read_u64().await?;
            let verbose_build = client.read_u64().await?;
            let log_type = client.read_u64().await?;
            let print_build_trace = client.read_u64().await?;
            let build_cores = if version >= ProtocolVersion::new(1, 10) {
                client.read_u64().await?
            } else {
                0
            };
            let use_substitutes = if version >= ProtocolVersion::new(1, 12) {
                client.read_u64().await?
            } else {
                0
            };
            let overrides = if version >= ProtocolVersion::new(1, 12) {
                let count = client.read_u64().await?;
                let mut ov = Vec::with_capacity(count as usize);
                for _ in 0..count {
                    let name = client.read_string().await?;
                    let value = client.read_string().await?;
                    ov.push((name, value));
                }
                ov
            } else {
        ...
        // No arguments
        Op::SyncWithGC | Op::FindRoots | Op::QueryAllValidPaths | Op::OptimiseStore => {
            Ok(Some(OpCall::NoArgs { op }))
        }

        // AddToStore (>= 1.25): name, camStr, refs, repair, then framed data
        Op::AddToStore => {
            if version >= ProtocolVersion::new(1, 25) {
                let name = client.read_string().await?;
                let cam_str = client.read_string().await?;
                let refs = client.read_string_set().await?;
                let repair = client.read_u64().await? != 0;
                let data = client.read_framed().await?;
                Ok(Some(OpCall::AddToStore {
                    name,
                    cam_str,
                    refs,
                    repair,
                    data: FramedData::Inline(data),
                }))
            } else {
                // Old protocol - fallback to raw bytes would be needed
                Ok(None)
            }
        }

        // AddMultipleToStore: repair, dontCheckSigs, then framed data
        Op::AddMultipleToStore => {
            let repair = client.read_u64().await? != 0;
            let dont_check_sigs = client.read_u64().await? != 0;
            let data = client.read_framed().await?;
            Ok(Some(OpCall::AddMultipleToStore {
                repair,
                dont_check_sigs,
                data: FramedData::Inline(data),
            }))
        ...
        // AddBuildLog: drvPath, then framed data
        Op::AddBuildLog => {
            let path = client.read_string().await?;
            let data = client.read_framed().await?;
            Ok(Some(OpCall::AddBuildLog {
                path,
                data: FramedData::Inline(data),
            }))
        }

        // BuildPaths / BuildPathsWithResults: DerivedPaths + mode
        Op::BuildPaths | Op::BuildPathsWithResults => {
            let paths = client.read_string_set().await?;
            let mode_val = client.read_u64().await?;
            ...
            Ok(Some(OpCall::PathSetMode { op, paths, mode }))
        }

        // QueryMissing: DerivedPaths
        Op::QueryMissing => {
            let paths = client.read_string_set().await?;
            Ok(Some(OpCall::PathSet { op, paths }))
        }
        ...
        Op::QuerySubstitutablePathInfo => {
            let path = client.read_string().await?;
            Ok(Some(OpCall::QuerySubstitutablePathInfo { path }))
        }

        // QuerySubstitutablePathInfos: StorePathSet
        Op::QuerySubstitutablePathInfos => {
            let paths = client.read_string_set().await?;
            Ok(Some(OpCall::QuerySubstitutablePathInfos { paths }))
        }

        // CollectGarbage
        Op::CollectGarbage => {
            let action = client.read_u64().await?;
            let paths = client.read_string_set().await?;
            let ignore_liveness = client.read_u64().await?;
            let max_freed = client.read_u64().await?;
            // obsolete fields
            client.read_u64().await?;
            client.read_u64().await?;
            client.read_u64().await?;
            Ok(Some(OpCall::CollectGarbage {
                action,
                paths,
                ignore_liveness,
                max_freed,
        ...
        // VerifyStore: checkContents, repair
        Op::VerifyStore => {
            let check_contents = client.read_u64().await? != 0;
            let repair = client.read_u64().await? != 0;
            Ok(Some(OpCall::VerifyStore {
                check_contents,
                repair,
            }))
        }
        ...
        Op::AddPermRoot => {
            let path = client.read_string().await?;
            let gc_root = client.read_string().await?;
            Ok(Some(OpCall::AddPermRoot { path, gc_root }))
        }

        // AddSignatures: path + sigs
        Op::AddSignatures => {
            let path = client.read_string().await?;
            let sigs = client.read_string_set().await?;
            Ok(Some(OpCall::AddSignatures { path, sigs }))
        }

        // RegisterDrvOutput
        Op::RegisterDrvOutput => {
            let value = client.read_string().await?;
            Ok(Some(OpCall::RegisterDrvOutput { value }))
        }

        // QueryRealisation
        Op::QueryRealisation => {
            let value = client.read_string().await?;
            Ok(Some(OpCall::QueryRealisation { value }))
        }

        // AddTextToStore: suffix, text, refs
        Op::AddTextToStore => {
            let suffix = client.read_string().await?;
            let text = client.read_string().await?;
            let refs = client.read_string_set().await?;
            Ok(Some(OpCall::AddTextToStore { suffix, text, refs }))
        }
...
/// Read the daemon's result data after STDERR_LAST for a given op.
///
/// Returns structured ResultData.
pub async fn read_daemon_result<D>(
    op: Op,
    version: ProtocolVersion,
    daemon: &mut AsyncWireReader<D>,
) -> Result<ResultData>
    ...
    D: AsyncBufRead + Unpin,
{
    match op {
        ...
        | Op::AddBuildLog
        | Op::BuildPaths => {
            let v = daemon.read_u64().await?;
            Ok(ResultData::U64(v))
        }

        // Single string
        Op::AddPermRoot | Op::AddTextToStore | Op::QueryDeriver | Op::QueryPathFromHashPart => {
            let s = daemon.read_string().await?;
            Ok(ResultData::Str(s))
        }
        ...
        Op::AddToStore => {
            if version >= ProtocolVersion::new(1, 25) {
                let _path = daemon.read_string().await?;
                let info = read_unkeyed_valid_path_info(daemon, version).await?;
                Ok(ResultData::PathInfo(PathInfoResult {
                    valid: true,
                    deriver: Some(info.deriver),
                    nar_hash: Some(info.nar_hash),
                    references: Some(info.references),
                    registration_time: Some(info.registration_time),
        ...
        // NarFromPath: framed data
        Op::NarFromPath => {
            if version >= ProtocolVersion::new(1, 23) {
                let data = daemon.read_framed().await?;
                Ok(ResultData::Framed(data))
            } else {
                Ok(ResultData::None)
            }
        }
        ...
        | Op::QueryDerivationOutputs
        | Op::QueryDerivationOutputNames => {
            let set = daemon.read_string_set().await?;
            Ok(ResultData::StringSet(set))
        }
        ...
        Op::QueryDerivationOutputMap => {
            ...
            for _ in 0..count {
                let name = daemon.read_string().await?;
                let path = daemon.read_string().await?;
                pairs.push((name, path));
            }
            Ok(ResultData::StringMap(pairs))
        }
        ...
        Op::FindRoots => {
            ...
            let mut pairs = Vec::with_capacity(count as usize);
            for _ in 0..count {
                let root = daemon.read_string().await?;
                let target = daemon.read_string().await?;
                pairs.push((root, target));
            }
            Ok(ResultData::StringMap(pairs))
        }
        ...
        // CollectGarbage
        Op::CollectGarbage => {
            let bytes_freed = daemon.read_u64().await?;
            daemon.read_u64().await?; // obsolete
            daemon.read_u64().await?; // obsolete
            Ok(ResultData::CollectGarbage { bytes_freed })
        }

        // QuerySubstitutablePathInfo
        Op::QuerySubstitutablePathInfo => {
            let valid = daemon.read_u64().await? != 0;
            if valid {
                let deriver = daemon.read_string().await?;
                let refs = daemon.read_string_set().await?;
                let download_size = daemon.read_u64().await?;
                let nar_size = daemon.read_u64().await?;
                Ok(ResultData::SubstitutablePathInfo {
                    valid: true,
        ...
        // QueryMissing
        Op::QueryMissing => {
            let will_build = daemon.read_string_set().await?;
            let will_substitute = daemon.read_string_set().await?;
            let unknown = daemon.read_string_set().await?;
            let download_size = daemon.read_u64().await?;
            let nar_size = daemon.read_u64().await?;
            Ok(ResultData::Missing {
                will_build,
                will_substitute,
                unknown,
                download_size,
        ...
        // BuildDerivation: too complex, skip it
        Op::BuildDerivation => {
            skip_build_result(daemon, version).await?;
            Ok(ResultData::None)
        }

        // BuildPathsWithResults: count + (DerivedPath + BuildResult)
        Op::BuildPathsWithResults => {
            let count = daemon.read_u64().await?;
            for _ in 0..count {
                daemon.skip_string().await?;
...
/// Read the stderr loop and capture error info if applicable.
pub async fn read_stderr_with_error<D>(
    daemon: &mut AsyncWireReader<D>,
) -> Result<(StderrResult, Option<ErrorInfo>)>
where
    D: AsyncBufRead + Unpin,
{
    let stderr_result = read_stderr_loop(daemon).await?;
    let error_info = if stderr_result.terminal == Some(StderrCode::Error) {
        let error_type = daemon.read_string().await?;
        let level = daemon.read_u64().await?;
        let name = daemon.read_string().await?;
        let message = daemon.read_string().await?;
        let have_pos = daemon.read_u64().await?;
        if have_pos != 0 {
            daemon.skip_string().await?; // file
            daemon.read_u64().await?; // line
            daemon.read_u64().await?; // column
        }
        Some(ErrorInfo {
            error_type,
            level,
    ...
    Ok((stderr_result, error_info))
}

/// Internal: parsed UnkeyedValidPathInfo fields.
struct ValidPathInfoFields {
    deriver: String,
    nar_hash: String,
...
/// Read UnkeyedValidPathInfo fields (does not skip, returns data).
async fn read_unkeyed_valid_path_info<D>(
    daemon: &mut AsyncWireReader<D>,
    version: ProtocolVersion,
) -> Result<ValidPathInfoFields>
where
    D: AsyncBufRead + Unpin,
{
    let deriver = daemon.read_string().await?;
    let nar_hash = daemon.read_string().await?;
    let references = daemon.read_string_set().await?;
    let registration_time = daemon.read_u64().await?;
    let nar_size = daemon.read_u64().await?;
    let (ultimate, sigs, ca) = if version >= ProtocolVersion::new(1, 16) {
        let ultimate = daemon.read_u64().await? != 0;
        let sigs = daemon.read_string_set().await?;
        let ca = daemon.read_string().await?;
        (Some(ultimate), Some(sigs), Some(ca))
    } else {
        (None, None, None)
    };
    Ok(ValidPathInfoFields {
        deriver,
        nar_hash,
        references,
        registration_time,
